...
df$generation <- ifelse(grepl("18-22", df$age), "zoom",
                 ifelse(grepl("23-28|29-35", df$age), "millennial", "other"))
...
chi_age <- chisq.test(t1)
print(chi_age)

cat("\n--- Хи-квадрат: Доход x Сортировка ---\n")
chi_inc <- chisq.test(t2)
print(chi_inc)

cat("\n--- Хи-квадрат: Образование x Сортировка ---\n")
chi_edu <- chisq.test(t3)
print(chi_edu)

# U-критерий Манна–Уитни
df_mw <- df[df$generation %in% c("zoom", "millennial"), ]
df_mw$q2_1_num <- score_freq(df_mw$q2_1_waste)
df_mw$q2_2_num <- score_freq(df_mw$q2_2_eco)
df_mw$q2_3_num <- score_freq(df_mw$q2_3_energy)

cat("\n--- U-критерий Манна–Уитни: Зумеры vs Миллениалы ---\n")
cat("Сортировка (2.1): ")
w1 <- wilcox.test(q2_1_num ~ generation, data = df_mw, exact = FALSE)
cat("W =", w1$statistic, ", p =", w1$p.value, "\n")
cat("Эко-товары (2.2): ")
w2 <- wilcox.test(q2_2_num ~ generation, data = df_mw, exact = FALSE)
cat("W =", w2$statistic, ", p =", w2$p.value, "\n")
cat("Экономия (2.3): ")
w3 <- wilcox.test(q2_3_num ~ generation, data = df_mw, exact = FALSE)
cat("W =", w3$statistic, ", p =", w3$p.value, "\n")
